<template>
    <div class="text-line" :dir="direction">
        <span class="line-name"
              ...
              @contextmenu="
                openLineMenu($event, 'popover-line-' + line.lineId)
            ">{{line.lineName}}</span>
      ...
      >
        ...
        <b-modal :id="`parallel-line-${line.lineId}`" hide-footer title="Parallel Texts from QWB">
        ...
        <b-modal :id="`variant-line-${line.lineId}`" hide-footer title="Variant Edition Transcriptions">
          <b class="line-name">Line {{line.lineName}}</b>
          <div class="text-line" :dir="direction">
            <text-sign :withMenu="true" :id="'popover-si-' + sign.signInterpretations[0].signInterpretationId" v-for="sign in line.signs" :key="sign.signInterpretations[0].signInterpretationId" :sign="sign"></text-sign>
          </div>
          ...
          <div v-if="variants.length > 0">
            <div v-for="variant in variants">
              ...
              <div :dir="direction">
                <text-sign :withMenu="false" v-for="sign in variant.signs" :key="`ed-${variant.editionId}-line-${sign.signInterpretations[0].signInterpretationId}`" :sign="sign"></text-sign>
              </div>
            </div>
          </div>
          <div v-if="variants.length === 0">
...
import { EditorDTO, LineDTO, QwbParallelListDTO, LineTextDTO, SignDTO } from '@/dtos/sqe-dtos';
...
export default class TextLineComponent extends Vue {
    ...
    private async showVariants() {
      ...
      if (this.variants.length === 0) {
        ...
        const variantEditionList = await es.getManuscriptEditions(currentEdition.primary.manuscriptId);
        const variantEditions = variantEditionList.editions.flatMap(x => x[0]);
        const ts = new TextService();
        for (const variantEdition of variantEditions) {
          if (variantEdition.id !== this.editionId) {
            try {
              const line = await ts.getLineText(variantEdition.id, this.line.lineId) as DetailedLineTextDTO;
